...
def download_cuad(split: str = "test") -> tuple[list[dict], list[dict]]:
    ...
    # Official Zenodo archive mirror for CUAD v1
    ZIP_URL = "https://zenodo.org/records/4595826/files/CUAD_v1.zip?download=1"
    logger.info("Downloading CUAD_v1.zip from Zenodo archive…")

    with tempfile.TemporaryDirectory() as tmp:
        zip_path = os.path.join(tmp, "cuad.zip")

        # Request with User-Agent header to ensure smooth redirects and download
        ...
        if not json_path:
            logger.error("Could not find JSON file in downloaded zip.")
            sys.exit(1)

        with open(json_path, encoding="utf-8") as f:
            raw = json.load(f)

    # Flatten nested SQuAD format → list of QA rows
    rows = []
    ...
    train_data, test_data = rows[:cut], rows[cut:]
    logger.info("Train: %d rows, Test: %d rows.", len(train_data), len(test_data))
...
def save_contracts(
    ...
    The new theatticusproject/cuad-qa format is flat: each row has a 'title'
    and a 'context' (one paragraph).  We group all contexts by title and write
    ...
    # Group paragraph contexts by contract title (flat SQuAD format)
    ...
    for row in raw_data:
        ...
        if context:
            by_title.setdefault(title, []).append(context)

    for title, paragraphs in by_title.items():
        safe_name = re.sub(r'[^\w\-.]', '_', title)[:120]
        file_path = contracts_dir / f"{safe_name}.txt"
        # Deduplicate paragraphs (same context can appear multiple times across QA pairs)
        seen = set()
        unique_paras = []
        for p in paragraphs:
            if p not in seen:
        ...
        file_path.write_text("\n\n".join(unique_paras), encoding="utf-8")
...
def build_eval_dataset(
    ...
    The new flat format has one QA pair per row:
      {
        "id": "...",
        "title": "ContractName",
        "context": "...",
        "question": "Highlight the parts related to 'Governing Law'...",
        "answers": {"text": ["Delaware law governs..."], "answer_start": [123]}
      }

    Unanswerable questions have answers["text"] == [].
    """
    ...
    for row in raw_data:
        ...
        # In the flat format, answers is a dict: {"text": [...], "answer_start": [...]}
        answer_texts = answers.get("text", [])

        # Skip unanswerable (empty answer list)
        if not answer_texts:
            continue
        ...
        category = _parse_category(question_raw)
        ...
        natural_q = _naturalise_question(question_raw, category)
